...
func (self *Terminal) NextEvent() InputEvent {
	b := nextByte(&self.stdin)
	if self.stdin.Buffered() > 0 && b == '\x1b' && isCtrlChar(peekByte(&self.stdin)) {
		return NewInputEventFromCtrlChar(&self.stdin)
	}
